...
class SponsorOrganization(UserProfile):
    OrganizationName = models.CharField(max_length = 30)
    pointConversion = models.DecimalField(default = 1.0, decimal_places= 2, max_digits= 7, blank=True)
    negativePoint = models.BooleanField(blank= True)
    address = models.TextField(max_length= 85, blank=True)
    phoneNumber = models.TextField(max_length=15, blank=True)

    def __str__(self):
        return self.user.username

    #Non Profile Objects
    #DriverUsers = models.ManyToManyField(SponsorList, related_name='sponsored_users')
    #sponsorUsers = models.ManyToManyField(SponsorList, related_name='sponsored_users')
