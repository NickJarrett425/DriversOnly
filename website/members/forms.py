...
from .models import UserProfile, DriverProfile, SponsorUserProfile, SponsorList, PointReason, SponsorOrganization
...
class UpdateOrganizationForm(forms.ModelForm):
    ...
    OrganizationName = forms.CharField(max_length=100, required= False)
    email = forms.EmailField(required= False)
    pointConvert = forms.DecimalField(required=False)
    negativePointToggle = forms.BooleanField(required= False)
    address = forms.CharField(required= False)
    PhoneNumber = forms.CharField(required= False)

    class Meta:
        model = SponsorOrganization
        fields = ['OrganizationName', 'email', 'pointConvert', 'negativePointToggle', 'address', 'PhoneNumber']
...
class OrgEmailForm(PasswordResetForm):
    pass
